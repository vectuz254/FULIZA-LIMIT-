...
// One photo is fetched per unique category and reused across every card in
// that category, so a page with 30 jobs across 6 categories only makes 6
// API calls, not 30.
const categoryPhotoCache = {};

const CATEGORY_QUERIES = {
  'Healthcare': 'black nurse hospital smiling',
  'Construction': 'black construction engineer site',
  'Engineering': 'black engineer working site',
  'Hospitality': 'black chef hotel kitchen',
  'IT': 'black software developer laptop office',
  'Technology': 'black software developer laptop office',
  'Education': 'black teacher classroom',
  'Logistics': 'black warehouse logistics worker',
  'Finance': 'black business professional office meeting',
  'Domestic': 'black housekeeper nanny smiling',
  'Sales': 'black sales professional office',
  'Admin': 'black office administrator desk',
};
const DEFAULT_QUERY = 'black professional working office smiling';

async function getCategoryPhoto(category) {
  const key = category || 'General';
  if (categoryPhotoCache[key] !== undefined) return categoryPhotoCache[key];
  const query = CATEGORY_QUERIES[key] || DEFAULT_QUERY;
  const photo = await fetchPexelsPhoto(query, 'square');
  categoryPhotoCache[key] = photo ? photo.src.medium : null;
  return categoryPhotoCache[key];
}

// After the job grid is drawn, quietly fetch one photo per unique category
// present on screen and fade each one into its matching cards.
async function loadJobPhotos(jobs) {
  const categories = [...new Set(jobs.map(j => j.category || 'General'))];
  await Promise.all(categories.map(async (cat) => {
    const url = await getCategoryPhoto(cat);
    if (!url) return;
    document.querySelectorAll(`.job-photo[data-category="${CSS.escape(cat)}"] img`).forEach(img => {
      img.src = url;
...
function renderJobs(jobs) {
  ...
  grid.innerHTML = jobs.map(j => `
    <div class="job-card reveal" data-category="${j.category}" data-country="${j.country}">
      <div class="job-photo" data-category="${j.category}">
